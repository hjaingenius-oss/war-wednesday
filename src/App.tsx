...
function Leaderboard() {
  ...
  <section className="card"><h2>Main Ranked Board</h2><div className="table-wrap"><table><thead><tr><th>Player</th><th>Score</th><th>Form</th><th>Total Points</th><th>Matches Played</th><th>Win %</th><th>KDA</th><th>K/D</th><th>Damage</th><th>Damage/Game</th><th>Headshot Kills</th><th>HS Kills/Game</th><th>10+ K</th><th>20+ K</th><th>30+ K</th><th>Knife Kills</th></tr></thead><tbody>
  {board.mainRows.map((r,i)=><tr key={r.playerId}><td><NavLink to={`/players/${r.playerId}`} className="player-link">{renderPlayerIdentity(r.name, playerTitles.get(r.playerId) || [], i)}</NavLink></td><td className="war-cell">{Math.round(r.warRating)}</td><td><span className="form-cell">{Math.round(r.formRating)}{r.smallSample ? ' *' : ''}</span></td><td>{Math.round(r.totalPoints)}</td><td>{r.matchesPlayed}</td><td>{fmt1(r.winPct)}%</td><td>{`${r.kills}/${r.deaths}/${r.assists}`}</td><td>{fmt1(r.kd)}</td><td>{Math.round(r.damage)}</td><td>{fmt1(r.damagePerGame)}</td><td>{Math.round(r.headshotKills)}</td><td>{fmt1(r.headshotKillsPerGame)}</td><td>{r.games10PlusKills}</td><td>{r.games20PlusKills}</td><td>{r.games30PlusKills}</td><td>{r.knifeKills}</td></tr>)}
  </tbody></table></div></section>
  <section className="card"><h2>Impact Board</h2><p className="muted">Highlights strong low-attendance and small-sample players without affecting official ranks.</p><div className="table-wrap"><table><thead><tr><th>Player</th><th>Score</th><th>Form</th><th>Total Points</th><th>Matches Played</th><th>Win %</th><th>KDA</th><th>K/D</th><th>Damage</th><th>Damage/Game</th><th>Headshot Kills</th><th>HS Kills/Game</th><th>10+ K</th><th>20+ K</th><th>30+ K</th><th>Knife Kills</th><th>Comparison</th></tr></thead><tbody>
...
function renderPlayerIdentity(name: string, titles: string[], rankIndex?: number) {
  ...
  return (
    <div className="player-id-cell">
      {typeof rankIndex === 'number' && <span className={`rank-chip rank-${rankIndex + 1}`}>{`#${rankIndex + 1}`}</span>}
