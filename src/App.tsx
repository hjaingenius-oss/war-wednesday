...
function Leaderboard() {
  ...
  <section className="card"><h2>Main Ranked Board</h2><div className="table-wrap"><table><thead><tr><th>Player</th><th>Score</th><th>Form</th><th>Total Points</th><th>Matches<br/>Played</th><th>Win %</th><th>KDA</th><th>K/D</th><th>Damage</th><th>UD</th><th>EF</th><th>DMG<br/>/Game</th><th>HS</th><th>HS<br/>/Game</th><th>10+ K</th><th>20+ K</th><th>30+ K</th><th>Knife<br/>Kills</th></tr></thead><tbody>
  {board.mainRows.map((r,i)=><tr key={r.playerId}><td><NavLink to={`/players/${r.playerId}`} className="player-link">{renderPlayerIdentity(r.name, playerTitles.get(r.playerId) || [], i)}</NavLink></td><td className="war-cell">{Math.round(r.warRating)}</td><td><span className="form-cell">{Math.round(r.formRating)}{r.smallSample ? ' *' : ''}</span></td><td>{Math.round(r.totalPoints)}</td><td>{r.matchesPlayed}</td><td>{fmt1(r.winPct)}%</td><td>{`${r.kills}/${r.deaths}/${r.assists}`}</td><td>{fmt1(r.kd)}</td><td>{Math.round(r.damage)}</td><td>{Math.round(r.utilityDamage)}</td><td>{Math.round(r.enemyFlashed)}</td><td>{fmt1(r.damagePerGame)}</td><td>{Math.round(r.headshotKills)}</td><td>{fmt1(r.headshotKillsPerGame)}</td><td>{r.games10PlusKills}</td><td>{r.games20PlusKills}</td><td>{r.games30PlusKills}</td><td>{r.knifeKills}</td></tr>)}
  </tbody></table></div></section>
  <section className="card"><h2>Impact Board</h2><p className="muted">Highlights strong low-attendance and small-sample players without affecting official ranks.</p><div className="table-wrap"><table><thead><tr><th>Player</th><th>Score</th><th>Form</th><th>Total Points</th><th>Matches<br/>Played</th><th>Win %</th><th>KDA</th><th>K/D</th><th>Damage</th><th>UD</th><th>EF</th><th>DMG<br/>/Game</th><th>HS</th><th>HS<br/>/Game</th><th>10+ K</th><th>20+ K</th><th>30+ K</th><th>Knife<br/>Kills</th><th>Comparison</th></tr></thead><tbody>
...
function MatchDetail() {
  ...
  const topUtilityDamage = mr.length ? Math.max(...mr.map((r)=>Number((r as MatchPlayer).utilityDamage || 0))) : 0;
  const topEnemyFlashed = mr.length ? Math.max(...mr.map((r)=>Number((r as MatchPlayer).enemyFlashed || 0))) : 0;
  ...
  return <div className="card"><h2>{normalizeMapName(m.map)} - {m.date}</h2><p><b>Match ID:</b> {getMatchDisplayId(m.id, matchDisplayIds)}</p><p>Final Score: {m.teamAName || 'Side A'} {m.teamAScore} - {m.teamBScore} {m.teamBName || 'Side B'}</p>
    ...
    <div className="helper-grid"><p><b>Highest Score</b><br/>{mr.find((r)=>calculateMatchValue(r, m, mr)===topMatchValue) ? `${playerName(players, mr.find((r)=>calculateMatchValue(r, m, mr)===topMatchValue)!.playerId)} (${fmt1(topMatchValue)})` : '-'}</p><p><b>Top Fragger</b><br/>{mr.find((r)=>r.kills===topKills) ? `${playerName(players, mr.find((r)=>r.kills===topKills)!.playerId)} (${topKills})` : '-'}</p><p><b>Highest Damage</b><br/>{mr.find((r)=>(r.damage||0)===topDamage) ? `${playerName(players, mr.find((r)=>(r.damage||0)===topDamage)!.playerId)} (${topDamage})` : '-'}</p><p><b>Highest Utility Damage</b><br/>{mr.find((r)=>(r as MatchPlayer).utilityDamage===topUtilityDamage) ? `${playerName(players, mr.find((r)=>(r as MatchPlayer).utilityDamage===topUtilityDamage)!.playerId)} (${topUtilityDamage})` : '-'}</p><p><b>Most Enemies Flashed</b><br/>{mr.find((r)=>(r as MatchPlayer).enemyFlashed===topEnemyFlashed) ? `${playerName(players, mr.find((r)=>(r as MatchPlayer).enemyFlashed===topEnemyFlashed)!.playerId)} (${topEnemyFlashed})` : '-'}</p><p><b>Best ADR</b><br/>{mr.find((r)=>(((r.damage||0)/(Math.max(1,m.teamAScore + m.teamBScore)))===topAdr)) ? `${playerName(players, mr.find((r)=>(((r.damage||0)/(Math.max(1,m.teamAScore + m.teamBScore)))===topAdr))!.playerId)} (${fmt1(topAdr)})` : '-'}</p></div>
    {teams.map((t)=><div key={t}><h3>{t}</h3><div className="table-wrap"><table><thead><tr><th>Player</th><th>Side</th><th>Result</th><th>K</th><th>D</th><th>A</th><th>Damage</th><th>UD</th><th>EF</th><th>HS%</th><th>ADR</th><th>K/D</th><th>Total Points</th><th>Score</th></tr></thead><tbody>{mr.filter((r)=>r.team===t).map((r)=><tr key={r.id} className={[calculateMatchValue(r, m, mr)===topMatchValue ? 'highlight-value' : '', r.kills===topKills ? 'highlight-kills' : '', safeKD(r.kills,r.deaths)===topKd ? 'highlight-kd' : '', (r.damage||0)===topDamage ? 'highlight-dmg' : '', ((r as MatchPlayer).utilityDamage||0)===topUtilityDamage ? 'highlight-dmg' : '', ((r as MatchPlayer).enemyFlashed||0)===topEnemyFlashed ? 'highlight-adr' : '', (((r.damage||0)/(Math.max(1,m.teamAScore + m.teamBScore)))===topAdr) ? 'highlight-adr' : ''].join(' ')}><td>{players.find((p)=>p.id===r.playerId)?.name}</td><td>{r.team}</td><td>{r.result}</td><td>{r.kills}</td><td>{r.deaths}</td><td>{r.assists}</td><td>{r.damage ?? '-'}</td><td>{(r as MatchPlayer).utilityDamage ?? '-'}</td><td>{(r as MatchPlayer).enemyFlashed ?? '-'}</td><td>{r.hsPercent ?? '-'}</td><td>{r.damage != null ? fmt1((r.damage)/(Math.max(1,m.teamAScore + m.teamBScore))) : '-'}</td><td>{safeKD(r.kills,r.deaths)}</td><td>{fmt(calculateTotalPointsForMatch(r))}</td><td className="war-cell">{fmt1(calculateMatchValue(r, m, mr))}</td></tr>)}</tbody></table></div></div>)}
  </div>;
}
...
function Players() {
  ...
  return <div className="cards">{board.map((p, i)=><NavLink key={p.playerId} to={`/players/${p.playerId}`} className="card player"><p className={`rank-chip rank-${i+1}`}>{`Rank #${i+1}`}</p><h3>{p.name}</h3><p>{fmt1(p.warRating)} Score</p><p>{p.matchesPlayed} matches</p><p>{p.category}</p><p>K/D {p.kd}</p><p>Knifed {p.knifeKills}</p></NavLink>)}</div>;
}
...
function PlayerProfile() {
  ...
  <section className="stats-grid"><div className="stat card"><h3>Utility Damage</h3><p>{Math.round(utilityDamage)}</p></div><div className="stat card"><h3>Enemies Flashed</h3><p>{Math.round(enemyFlashed)}</p></div></section>
...
function renderPlayerIdentity(name: string, titles: string[], rankIndex?: number) {
