  ...
  return <>
    ...
    <section className="card">
      ...
      <div className="awards-grid">
        {fun.awards.filter((a) => a.label === 'Assassin' || a.label === 'Assist Hero' || a.label === 'Wildcard').slice(0, 3).map((a) => <article className="award-card" key={a.key}>
...
function Leaderboard() {
  ...
  <section className="card"><h2>Main Ranked Board</h2><div className="table-wrap"><table><thead><tr><th>Rank</th><th>Player</th><th>Score</th><th>Form</th><th>Titles</th><th>Total Points</th><th>Matches Played</th><th>Win %</th><th>K/D</th><th>Kills</th><th>Deaths</th><th>Assists</th><th>Damage</th><th>Damage/Game</th><th>Headshot Kills</th><th>HS Kills/Game</th><th>10+ K</th><th>20+ K</th><th>30+ K</th><th>Knife Kills</th></tr></thead><tbody>
  {board.mainRows.map((r,i)=><tr key={r.playerId}><td><span className={`rank-chip rank-${i+1}`}>{`Rank #${i+1}`}</span></td><td><NavLink to={`/players/${r.playerId}`}>{r.name}</NavLink></td><td className="war-cell">{fmt1(r.warRating)}</td><td><span className="form-cell">{fmt1(r.formRating)}{r.smallSample ? ' *' : ''}</span></td><td>{renderTitleChips(playerTitles.get(r.playerId) || [])}</td><td>{fmt(r.totalPoints)}</td><td>{r.matchesPlayed}</td><td>{fmt1(r.winPct)}%</td><td>{r.kd}</td><td>{r.kills}</td><td>{r.deaths}</td><td>{r.assists}</td><td>{Math.round(r.damage)}</td><td>{fmt1(r.damagePerGame)}</td><td>{fmt1(r.headshotKills)}</td><td>{fmt1(r.headshotKillsPerGame)}</td><td>{r.games10PlusKills}</td><td>{r.games20PlusKills}</td><td>{r.games30PlusKills}</td><td>{r.knifeKills}</td></tr>)}
  </tbody></table></div></section>
  <section className="card"><h2>Impact Board</h2><p className="muted">Highlights strong low-attendance and small-sample players without affecting official ranks.</p><div className="table-wrap"><table><thead><tr><th>Player</th><th>Score</th><th>Form</th><th>Titles</th><th>Total Points</th><th>Matches Played</th><th>Win %</th><th>K/D</th><th>Kills</th><th>Deaths</th><th>Assists</th><th>Damage</th><th>Damage/Game</th><th>Headshot Kills</th><th>HS Kills/Game</th><th>10+ K</th><th>20+ K</th><th>30+ K</th><th>Knife Kills</th><th>Comparison</th></tr></thead><tbody>
  {board.impactRows.map((r)=><tr key={r.playerId}><td><NavLink to={`/players/${r.playerId}`}>{r.name}</NavLink></td><td className="war-cell">{fmt1(r.warRating)}</td><td><span className="form-cell">{fmt1(r.formRating)}{r.smallSample ? ' *' : ''}</span></td><td>{renderTitleChips(playerTitles.get(r.playerId) || [])}</td><td>{fmt(r.totalPoints)}</td><td>{r.matchesPlayed}</td><td>{fmt1(r.winPct)}%</td><td>{r.kd}</td><td>{r.kills}</td><td>{r.deaths}</td><td>{r.assists}</td><td>{Math.round(r.damage)}</td><td>{fmt1(r.damagePerGame)}</td><td>{fmt1(r.headshotKills)}</td><td>{fmt1(r.headshotKillsPerGame)}</td><td>{r.games10PlusKills}</td><td>{r.games20PlusKills}</td><td>{r.games30PlusKills}</td><td>{r.knifeKills}</td><td><span className="comparison-badge">{r.comparisonBadge}</span></td></tr>)}
